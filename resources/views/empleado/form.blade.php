<div class="box box-info padding-1">
    <div class="box-body">
        ...
        <div class="form-group">
            ...
            {!! $errors->first('persona_id', '<div class="invalid-feedback">Persona no válida</div>') !!}
        </div>
        <div class="form-group">
            ...
            {!! $errors->first('usuario_id', '<div class="invalid-feedback">Usuario no válido</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('cargo_id') }}
            {{ Form::select('cargo_id', $cargo, $empleado->cargo_id, ['class' => 'form-control' . ($errors->has('cargo_id') ? ' is-invalid' : ''), 'placeholder' => 'Cargo Id']) }}
            {!! $errors->first('cargo_id', '<div class="invalid-feedback">Cargo no válido</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('fecha_contratacion') }}
            {{ Form::date('fecha_contratacion', $empleado->fecha_contratacion, ['class' => 'form-control' . ($errors->has('fecha_contratacion') ? ' is-invalid' : ''), 'placeholder' => 'Fecha Contratación']) }}
            {!! $errors->first('fecha_contratacion', '<div class="invalid-feedback">Fecha no válida</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('sueldo') }}
            {{ Form::text('sueldo', $empleado->sueldo, ['class' => 'form-control' . ($errors->has('sueldo') ? ' is-invalid' : ''), 'placeholder' => 'Sueldo']) }}
            {!! $errors->first('sueldo', '<div class="invalid-feedback">Sueldo no válido</div>') !!}
        </div>
